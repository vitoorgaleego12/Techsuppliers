...
def get_db_connection():
    ...
    try:
        # Para Render, use a variável de ambiente DATABASE_URL
        ...
        if database_url:
            ...
            conn = psycopg2.connect(
                ...
                database=os.environ.get('DB_NAME', 'techsuppliers'),
...
def add_security_headers(response):
    ...
    if 'Set-Cookie' in response.headers:
        ...
        if 'HttpOnly' not in cookies:
            cookies += '; HttpOnly'
        if 'Secure' not in cookies and request.is_secure:
            cookies += '; Secure'
...
def serve_static(filename):
    ...
    response = send_from_directory('.', filename, mimetype=mimetype)
...
@csrf.exempt  # Exempt para login, mas validação manual abaixo
@rate_limit(max_requests=10, window=60)
def login_cliente():
    try:
        # Verificação manual do CSRF token
        csrf_token = request.headers.get('X-CSRFToken') or request.form.get('csrf_token')
        if not csrf_token or not csrf.validate_csrf_token(csrf_token):
            return jsonify({"status": "erro", "mensagem": "Token CSRF inválido."}), 400
...
@csrf.exempt  # Validação manual do CSRF
@rate_limit(max_requests=10, window=60)
...
def cadastrar_cliente():
    try:
        ...
        session['cliente_email'] = novo_cliente[2]
        session['logged_in'] = True
...
def health_check():
    try:
        ...
        cursor.close()
        conn.close()
        return jsonify({"status": "healthy", "database": "connected", "timestamp": time()})
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"🛡️  CSRF Protection ativado")
    ...
    def decorated_function(*args, **kwargs):
        if not session.get('logged_in'):
            return jsonify({"status": "erro", "mensagem": "Login necessário para acessar este recurso."}), 401
        return f(*args, **kwargs)
    return decorated_function
...
def serve_static(filename):
    ...
    # Adiciona headers de cache para arquivos estáticos
    response = send_from_directory(os.path.dirname(os.path.abspath(__file__)), filename, mimetype=mimetype)
    response.headers['Cache-Control'] = 'public, max-age=3600'
    return response
...
@app.route("/fornecedores")
def pagina_fornecedores():
    return send_from_directory(os.path.dirname(os.path.abspath(__file__)), "cadastro.html")

@app.route("/clientes")
def pagina_clientes():
    return send_from_directory(os.path.dirname(os.path.abspath(__file__)), "cliente.html")

@app.route("/listar_fornecedores")
@login_required
def listar_fornecedores():
    return send_from_directory(os.path.dirname(os.path.abspath(__file__)), "Listar.html")

@app.route("/listar_clientes")
@login_required
def listar_clientes():
    return send_from_directory(os.path.dirname(os.path.abspath(__file__)), "Listar_clientes.html")
...
def cadastrar_cliente():
    try:
        ...
        # Cria a sessão do usuário automaticamente após o cadastro
        session['cliente_id'] = novo_cliente[0]  # ID
...
if __name__ == "__main__":
    ...
    if os.environ.get('RENDER'):
        ...
        # No Render, o PORT é fornecido automaticamente
...
def get_db_connection():
    """Retorna uma conexão segura com o banco PostgreSQL"""
    try:
        conn = psycopg2.connect(DATABASE_URL)
        return conn
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"🚀 Servidor TechSuppliers iniciado com PostgreSQL!")
    print(f"🔒 Modo de produção: {not os.environ.get('DEBUG', 'False').lower() in ('true', '1', 'yes')}")
