...
# ==============================
# Configuração de Content Security Policy (CSP) para imagens externas
# ==============================
@app.after_request
def add_security_headers(response):
    """Adiciona headers de segurança HTTP com CSP permitindo imagens externas"""
    # Lista de domínios permitidos para imagens (ajuste conforme necessário)
    image_sources = "'self' data: https: http:"
    
    # Se você sabe os domínios específicos, pode listá-los:
    # image_sources = "'self' data: https://exemplo.com https://outro-site.com"
    ...
    # CSP permitindo imagens de qualquer fonte (https e http)
...
# ==============================
# Configuração adicional para servir imagens corretamente
# ==============================
@app.route('/images/<path:filename>')
def serve_images(filename):
    """Serve imagens com headers apropriados"""
    return send_from_directory(os.path.join(PASTA_PROJETO, 'images'), filename)

# Adicione também extensões de imagem permitidas
ALLOWED_EXTENSIONS = {'.css', '.js', '.html', '.png', '.jpg', '.jpeg', '.gif', '.ico', '.svg', '.webp', '.bmp'}
...
if __name__ == "__main__":
    ...
    print(f"🚀 Servidor TechSuppliers iniciado com segurança máxima!")
    print(f"📍 URL: http://localhost:{porta_livre}")
    print(f"🌐 Imagens externas habilitadas na CSP")
    print(f"🔒 Headers de segurança configurados")
